목표: 이 저장소의 `grid_map_grit_slam` 패키지에 ROS 2 Humble용 로컬 맵 실행 경로를 구현해라.
- 입력: `/lidar/cloud_combined` (`sensor_msgs/msg/PointCloud2`, 로컬 맵용 포인트클라우드)
- 처리: 입력 PointCloud2를 기반으로 `GridMap` 생성
- 실행: `ros2 launch grid_map_grit_slam local_map_launch.py`
- 결과: Foxglove에서 바로 볼 수 있도록 `GridMap` 원본과 함께 `OccupancyGrid` 또는 `PointCloud2` 형태의 시각화 토픽도 함께 publish

필수 요구사항:

1. 기존 `grid_map_grit_slam` 패키지의 로컬 맵 실행 경로 구성
2. C++ bridge node 작성 또는 기존 bridge node 재사용 가능 구조로 정리
	- `/lidar/cloud_combined` subscribe
...
3. 단독 launch `local_map_launch.py` 작성 또는 정리
	- bridge node 실행
...
- input topic 기본값: `/lidar/cloud_combined`
...
- `grid_map_grit_slam/launch/local_map_launch.py`
...
2. `ros2 launch grid_map_grit_slam local_map_launch.py` 실행 가능
3. `/lidar/cloud_combined` 이 들어오면 아래 토픽 확인
	- `/grid_map`
	- `/elevation_points`
	- `/elevation_grid`
4. 각 토픽 타입이 기대와 일치
